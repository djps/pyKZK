...
def WAKZK_planar():
    ...
    # dummy layer
    #Layer[1] = LayerClass(0.0, 1482.0, 1000.0, 0.217, 1.0, 2.0, 3.5, 4180.0, 0.6, 0.0, 0)
    ...
    # some reporting
    if (output):
        print('\n\tWavelength = %8.2f [mm]' % np.float64(10.0*lambda0) )
        print('\tNode count')
        print('\t\tAxial %d' % Grid.NN)
        print('\t\tRadial %d' % Grid.JJ)
        print('\tGrid stepsize')
        print('\t\tdz = %3.2f [mm]' % (10.0*dz) )
        print('\t\tdr = %3.2f [mm]' % np.float64(10.0*dr))
    ...
    if (II > 1):
        for ii in np.arange(1,II-1):
            Layer[ii].index = int( np.round((Layer[ii].z - z_start)/dz)) #+ int(1)
        # do last
            ...
            if (ll < LL):
                ...
                if ( (Grid.z[nn+1] > z_output[ll]) & (Grid.z[nn] <= z_output[ll]) ):
                    ...
                    if ((hd == 0) or (np.abs(hd) < 10E-8)):
                        ...
                        if (debug): print("here 3: \t", ll, LL, z_output[ll], nn, Grid.z[nn], hd, np.shape(Grid.r), np.shape(p), hr, JJ_ )
    ...
    if (willPlot):
        ...
        if (debug):
            ...
            print( np.shape( p5 ), np.shape(SpecOut[0].p5), np.size(SpecOut[0].p5[:,0])-1, np.shape(Grid.r), MM, HH )
            print( np.shape( Grid.r[0:np.size(SpecOut[0].p5[:,0])-1] ) )
        ...
        for ll in np.arange(0, LL):
            ...
            #for jj in np.arange(0, np.shape(SpecOut[0].p5)[1], dtype=int):
            for jj in np.arange(0, np.min( np.shape(SpecOut[0].p5) ), dtype=int):
                #print( np.shape(z_output[ll]*r_ones), np.shape(Grid.r[0:np.size(SpecOut[ll].p5[:,jj])]), np.shape(SpecOut[ll].p5[:,jj]/1e6) )
        ...
        for ll in np.arange(0, LL):
            ...
            print( np.shape(Grid.r), np.shape(SpecOut[ll].p5[:,0]), np.size(SpecOut[ll].p5[:,0]) )
            print( np.shape( Grid.r[0:np.size(SpecOut[ll].p5[:,0])] ) )
            print( np.size(SpecOut[0].p5[0,:]), np.shape(SpecOut[0].p5[0,:]), MM, HH, np.min( np.shape(SpecOut[0].p5) ) )
            for jj in np.arange(0, np.size(SpecOut[0].p5[0,:]), dtype=int):
