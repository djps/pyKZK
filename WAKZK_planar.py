...
def WAKZK_planar():
    ...
    if (II > 1):
        for ii in np.arange(1,II-1):
            Layer[ii].index = np.int( np.round((Layer[ii].z - z_start)/dz)) #+ np.int(1)
        # do last
    ...
    if (willPlot):
        ...
        for ll in np.arange(0, LL):
            ...
            #for jj in np.arange(0, np.shape(SpecOut[0].p5)[1], dtype=np.int):
            for jj in np.arange(0, np.min( np.shape(SpecOut[0].p5) ), dtype=np.int):
        ...
        for ll in np.arange(0, LL):
            ...
            for jj in np.arange(0, np.size(SpecOut[0].p5[0,:]), dtype=np.int):
